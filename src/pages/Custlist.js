...
export const Custlist = ({upDate,setUpdate,performTask,setTaskperform}) => {
 ...
 const navigate = useNavigate()
  ...
  <div className="flex flex-wrap gap-6">
    {performTask.length > 0 ? (
      performTask.map((customer) => (
              ...
              <ul className="list-disc list-inside pl-5">
                {customer.add.map((address) => (
                  <li key={address.id} className="mb-4">
                    <p className="text-gray-600"><strong>Address line 1:</strong> {address.add1 || 'N/A'}</p>
                    <p className="text-gray-600"><strong>Address line 2:</strong> {address.add2 || 'N/A'}</p>
                    <p className="text-gray-600"><strong>Zip/Postal code:</strong> {address.pin || 'N/A'}</p>
                    <p className="text-gray-600"><strong>City:</strong> {address.city || 'N/A'}</p>
                    <p className="text-gray-600"><strong>State/Province:</strong> {address.state || 'N/A'}</p>
                  </li>
                ))}
              </ul>
            ) : (
